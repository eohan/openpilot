						       ...
						       uint32_t utc_date, uint32_t utc_time)
{
	...
	put_uint32_t_by_index(utc_date, 0,  msg->payload); // GPS UTC date ddmmyy
	put_uint32_t_by_index(utc_time, 4,  msg->payload); // GPS UTC time hhmmss
...
#else
	memcpy(system_time_utc, msg->payload, 8);
#endif
}
