						       ...
						       uint8_t target_system, uint8_t target_component, uint8_t command, uint8_t confirmation, float param1, float param2, float param3, float param4, float param5, float param6, float param7)
{
	...
	put_float_by_index(param5, 16,  msg->payload); // Parameter 5, as defined by MAV_CMD enum.
	put_float_by_index(param6, 20,  msg->payload); // Parameter 6, as defined by MAV_CMD enum.
	put_float_by_index(param7, 24,  msg->payload); // Parameter 7, as defined by MAV_CMD enum.
	put_uint8_t_by_index(target_system, 28,  msg->payload); // System which should execute the command
	put_uint8_t_by_index(target_component, 29,  msg->payload); // Component which should execute the command, 0 for all components
	put_uint8_t_by_index(command, 30,  msg->payload); // Command ID, as defined by MAV_CMD enum.
	put_uint8_t_by_index(confirmation, 31,  msg->payload); // 0: First transmission of this command. 1-255: Confirmation transmissions (e.g. for kill command)

	return mavlink_finalize_message(msg, system_id, component_id, 32, 77);
}
						           ...
						           uint8_t target_system,uint8_t target_component,uint8_t command,uint8_t confirmation,float param1,float param2,float param3,float param4,float param5,float param6,float param7)
{
	...
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 32, 77);
}
						           ...
						           uint8_t target_system,uint8_t target_component,uint8_t command,uint8_t confirmation,float param1,float param2,float param3,float param4,float param5,float param6,float param7)
{
	...
	mavlink_finalize_message_chan_send(msg, chan, 32, 77);
}
...
#else
	memcpy(command_long, msg->payload, 32);
#endif
}
